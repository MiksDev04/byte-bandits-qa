                  ...
                  <?php for ($i = 0; $i < 3; $i++): ?>
                    <div class="p-3" style="border-bottom:1px solid var(--border-light);">
                      ...
                      <div class="progress-bar-wrap mt-2">
                        <div class="progress-bar-fill blue" style="width:45%;"></div>
                      </div>
                    </div>
                  <?php endfor; ?>
                </div>
  ...
  <script>
    $(function() {
      ...
      function renderAudits(rows) {
        ...
        if (!rows.length) {
          tbody.html(emptyRow(9, 'No audits found'));
          return;
        }
        rows.forEach(r => {
          tbody.append(`<tr>
      ...
      function renderTasks(rows) {
        ...
        if (!rows.length) {
          tbody.html(emptyRow(7, 'No tasks found'));
          return;
        }
        rows.forEach(r => {
          tbody.append(`<tr>
      ...
      function renderSurveys(rows) {
        ...
        if (!rows.length) {
          tbody.html(emptyRow(9, 'No surveys found'));
          return;
        }
        rows.forEach(r => {
          tbody.append(`<tr>
      ...
      ────────────────────────────────────────────────────────── */
      function loadPlans() {
        if (cache['action_plans']) {
          renderPlans(cache['action_plans']);
          return;
        }
      ...
      function renderPlans(rows) {
        ...
        if (!rows.length) {
          tbody.html(emptyRow(7, 'No action plans found'));
          return;
        }
        rows.forEach(r => {
          tbody.append(`<tr>
      ...
      function loadStandards() {
        if (cache['standards']) {
          renderStandards(cache['standards']);
          return;
        }
      ...
      function renderStandards(rows) {
        ...
        if (!rows.length) {
          tbody.html(emptyRow(8, 'No standards found'));
          return;
        }
        rows.forEach(r => {
          tbody.append(`<tr>
      ...
      async function runExcelExport(selectedSections, dateFrom, dateTo) {
        ...
        if (selectedSections.includes('kpis')) {
          ...
          const filteredKpis = data.map(ind => {
            ...
            // AFTER
            const filteredRecords = (ind.records || []).filter(rec => {
              // school_year is like "2025 - 2026"; extract the start year for comparison
              const y = String(rec.school_year || '').split(' - ')[0].trim();
              if (!y) return false;
