...
<head>
    ...
    <style>
        ...
        .existing-doc-link:hover { text-decoration: underline; }
    </style>
</head>
...
<script>
    ...
    function loadStandards() {
        $.ajax({
            ...
            type: 'GET',
            data: { search: $('#searchInput').val(), status: $('#statusFilter').val() },
            dataType: 'json',
            success: function (r) {
                r.success ? renderStandards(r.data) : $('#standardsList').html('<div class="alert alert-warning">Failed to load standards</div>');
            },
            error: function () { $('#standardsList').html('<div class="alert alert-danger">Error loading standards</div>'); }
        });
    }

    function renderStandards(standards) {
        if (!standards.length) { $('#standardsList').html('<div class="text-center py-5 text-muted">No standards found</div>'); return; }
    ...
    function loadPolicies() {
        $.ajax({
            ...
            error: function () { $('#policiesList').html('<div class="alert alert-danger">Error loading policies</div>'); }
        });
    }

    function renderPolicies(policies) {
        if (!policies.length) { $('#policiesList').html('<div class="text-center py-5 text-muted">No policies found</div>'); return; }
    ...
    function loadStandardsForDropdown() {
        $.ajax({
            url: '../../backend/api/standards_api.php?action=list',
            type: 'GET', data: { status: 'Active' }, dataType: 'json',
            success: function (r) {
                if (r.success) {
                    let opts = '<option value="">-- None / General Policy --</option>';
                    r.data.forEach(s => { opts += `<option value="${s.standard_id}">${escapeHtml(s.title)}</option>`; });
    ...
    function saveStandard() {
        ...
        $.ajax({
            ...
            type: 'POST', data: JSON.stringify(formData),
            ...
            success: function (r) {
                ...
                if (r.success) {
                    ...
                    loadStandards(); loadStandardsForDropdown();
                } else {
                    if (r.errors) applyServerErrors('#standardForm', r.errors);
                    toast.error(r.message);
                }
            },
            error: function () { btnReset(btn[0]); toast.error('An error occurred'); }
        });
    }
        ...
        $.ajax({
            ...
            success: function (r) {
                if (r.success && r.data) {
                    ...
                    clearFormErrors('#standardForm');
                } else { toast.error('Failed to load standard data'); }
            },
            error: function () { toast.error('Error loading standard'); }
        });
    }
        ...
        $.ajax({
            ...
            success: function (r) {
                if (r.success && r.data) {
                    ...
                    clearFormErrors('#policyForm');
                } else { toast.error('Failed to load policy data'); }
            },
            error: function () { toast.error('Error loading policy'); }
        });
    }

    /* ── Delete ─────────────────────────────────────────────────────────── */
    function confirmDelete(type, id) { deleteType = type; deleteId = id; $('#deleteModal').modal('show'); }
        ...
        $.ajax({
            ...
            success: function (r) {
                ...
                if (r.success) {
                    ...
                    toast.success(r.message);
                    if (deleteType === 'standard') { loadStandards(); loadStandardsForDropdown(); loadPolicies(); }
                    else { loadPolicies(); }
                    deleteType = null; deleteId = null;
                } else { toast.error(r.message); }
            },
            error: function () { btnReset(btn[0]); toast.error('Delete failed'); }
        });
    });
