...
/**
 * Auto-update audit status when all its tasks are completed
 */
function syncAuditCompletionStatus($auditId, $conn)
{
    // Count total tasks and completed tasks for this audit
    $stmt = $conn->prepare(
        "SELECT 
            COUNT(*) AS total,
            SUM(status = 'Completed') AS completed
         FROM qa_accreditation_tasks
         WHERE audit_id = ?"
    );
    $stmt->bind_param("i", $auditId);
    $stmt->execute();
    ...
    // Only act when there is at least one task and all are completed
    if ($row['total'] > 0 && (int)$row['total'] === (int)$row['completed']) {
        $today = date('Y-m-d');
        $stmt = $conn->prepare(
            "UPDATE qa_audits
             SET status = 'Completed',
                 completion_date = COALESCE(completion_date, ?)
             WHERE audit_id = ? AND status != 'Completed'"
        );
        $stmt->bind_param("si", $today, $auditId);
        $stmt->execute();
    } else {
        // At least one task is not done — revert audit if it was auto-completed
        $stmt = $conn->prepare(
            "UPDATE qa_audits
         SET status = 'In Progress', completion_date = NULL
         WHERE audit_id = ? AND status = 'Completed'"
        );
        $stmt->bind_param("i", $auditId);
        $stmt->execute();
...
function createTask()
{
    ...
    if ($stmt->execute()) {
        ...
        syncAuditCompletionStatus($auditId, $conn);   // ← add
...
function updateTask()
{
    ...
    // AFTER
    if ($stmt->execute()) {
        syncAuditCompletionStatus($auditId, $conn);   // ← add (runs even on "no changes")
        if ($stmt->affected_rows > 0) {
